...
if (Test-Path "./save/init.sql") {
    ...
    Write-Host "    -> Exécution de l'import (Forçage binaire Linux)..."
    docker exec -e LANG=C.UTF-8 mariadb sh -c "mariadb -umathias -proot --default-character-set=utf8mb4 sae < /tmp/init.sql"
    # Nettoyage
    docker exec mariadb sh -c "rm -f /tmp/init.sql"
